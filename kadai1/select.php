...
$pdo = connectToDb();
//1. DB接続  insert.phpのDB接続と同じ（コピペ）
// $dbn = 'mysql:dbname=gsacfd04_db11;charset=utf8;port=3306;host=localhost';
// $user = 'root';
// $pwd = '';
...
if ($status == false) {
  ...
  while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $view .= '<a href="' . $result['url'] . '">' . $result['url'] . '</a>' . '-' . '<p>' . $result['name'] . '</p>';
  ...
  <header>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          <li class="nav-item">
            <a class="nav-link" href="index.php">書籍データ登録</a>
          </li>
        </ul>
      </div>
    </nav>
